...
public class Grids {
    ...
    public static int[] getPoint(final int index) {
        ...
        final int corner_number = (int) (Math.ceil((float) (higher_sqrt * higher_sqrt - lower_sqrt * lower_sqrt) / 2) + lower_sqrt * lower_sqrt);
        final int diff = index - corner_number;
        if (higher_sqrt % 2 == 0) {
            point[0] = layers + ((diff <= 0) ? 1 : 1 + (corner_number - index));
            point[1] = -layers - ((diff >= 0) ? 1 : 1 - (corner_number - index));
        } else {
            point[0] = -layers - ((diff <= 0) ? 1 : 1 + (corner_number - index));
            point[1] = layers + ((diff >= 0) ? 1 : 1 - (corner_number - index));
        }
